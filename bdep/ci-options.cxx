...
namespace bdep
{
  ...
  print_bdep_ci_usage (::std::ostream& os, ::bdep::cli::usage_para p)
  {
    ...
    os << "\033[1mSYNOPSIS\033[0m" << ::std::endl
       ...
       << ::std::endl
       << "Manifest overrides override the entire value group that they belong to." << ::std::endl
       << "Currently, the following value groups can be overridden. The \033[1mbuild-*email\033[0m group" << ::std::endl
       << "is overridden by default as if by specifying an empty build email." << ::std::endl
       << ::std::endl
       ...
       << "*-build-email *-build-{warning,error}-email" << ::std::endl
       << ::std::endl
       << "For the package configuration-specific build constraint and email overrides the" << ::std::endl
       << "corresponding configuration must exist in the package manifest. In contrast," << ::std::endl
       << "the package configuration override (\033[1m*-build-config\033[0m) adds a new configuration if" << ::std::endl
       << "it doesn't exist and updates the arguments of the existing configuration" << ::std::endl
       << "otherwise. In the former case, all the potential build constraint and email" << ::std::endl
       ...
       << "configuration-specific build emails group value overrides are mutually" << ::std::endl
       << "exclusive. If the common build emails are overridden, then all the" << ::std::endl
       << "configuration-specific emails are removed. Otherwise, if any" << ::std::endl
       << "configuration-specific emails are overridden, then for the remaining" << ::std::endl
       << "configurations the build emails are reset to the empty values and the build" << ::std::endl
       << "warning and error emails are removed (which effectively disables email" << ::std::endl
